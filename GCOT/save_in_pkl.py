...
def segment_to_keyinfo(content, output_dir, episode_id):
    instruction, images, segment_count = content
    # print(instruction, images, segment_count)
    # print(type(instruction), type(images), type(segment_count))

    pkl_file = os.path.join(output_dir, f"tuple_data_{episode_id}.pkl")
    with open(pkl_file, 'wb') as f:
        pickle.dump(content, f)

    print(f"file is saved in {pkl_file}")
    prompt = (
...
def main(split, load_prev=None):
    ...
    output_dir = os.path.join(os.getcwd(), "pickles")
    ...
    for i in range(len(dataset)):
        ...
        segment_to_keyinfo(outputs, output_dir, episode_id)
        # model_outputs = segment_to_keyinfo(output)
